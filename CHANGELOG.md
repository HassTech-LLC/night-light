...
- **Unattended `/S` install showed a dialog and tried to open a browser.** Five of the six installer dialogs had no `/SD` silent-mode default, so NSIS displayed them during a silent install. Measured in Windows Sandbox, which ships without the Microsoft Edge WebView2 Runtime and therefore fails the installer's precondition: the published build put a visible window on screen, spawned SmartScreen as it tried to open the download page, and took 11 to 30 seconds across two runs. With silent defaults it shows no window, spawns nothing, and exits in about a second. Both correctly decline to install. Whether the dialog can block a deployment indefinitely depends on the environment and was not observed here, so no such claim is made. The WebView2 prompt now declines by default, and the remaining stop dialogs default to OK. The contract test previously scanned only the install section, which is why it missed the four dialogs in `.onInit`; it now scans the whole script and is mutation-checked.
...
- Windows Night Light detection now decodes the CloudStore record as Microsoft Bond CompactBinary, where the presence of inner field 0 means on, instead of treating byte 18 as a state marker. That byte is the inner payload length, and paired live captures on Windows 11 build 26200 show three lengths on one machine for two states: 0x10 off before the switch has ever been toggled, 0x15 on, and 0x13 off afterwards, because Windows adds an initialized field once toggled. The previous code happened to classify all three correctly, so this is hardening against a latent failure rather than a fix for an observed one: any build emitting a length outside the known set, such as 0x12 for an on state without the initialized field, would have been reported as unknown, which pauses the filter indefinitely. Structures the parser cannot account for still fail safe as unknown.
